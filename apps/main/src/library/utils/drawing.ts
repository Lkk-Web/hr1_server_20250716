...
import { Drawing } from '@model/io/drawing.model'
...
export class DrawingTool{
  ...
  public static async getDrawingList(name:string,filetype:string,token:string): Promise<Array<any>> {
    ...
    const {data} = await axios.post(this.url+"api/event",{
      "Application": "Mkanban",
      "Behavior": "Query",
      "Method": "SearchDoc",
      "SecurityContext": {
  ...
  public static getDrawingDwgUrl(param:DrawingItem,token:string): string {
    const fileKey = param.FileKey
    const fileType = param.FileExt
    return `mkanban://${this.url.replace(":","")}api/file?filekey=${encodeURIComponent(fileKey)}&filetype=${fileType}&token=${token}${encodeURIComponent(`|${param.FileName}|0|${param.FileTypeNo}`)}`
  }
