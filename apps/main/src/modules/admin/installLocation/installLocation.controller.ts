...
@ApiTags('安装地点')
@ApiBearerAuth()
@AdminAuth('installLocation')
export class InstallLocationController {
  constructor(private readonly service: InstallLocationService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: CInstallLocationDTO, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: UInstallLocationDTO, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
