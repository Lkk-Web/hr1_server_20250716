...
@ApiTags('出库单')
@ApiBearerAuth()
@AdminAuth('outboundOrder')
export class OutboundOrderController {
  constructor(private readonly service: OutboundOrderService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: COutboundOrderDto, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: UOutboundOrderDto, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
