...
@ApiTags('检验单')
@ApiBearerAuth()
@AdminAuth('checkOrder')
export class CheckOrderController {
  constructor(private readonly service: CheckOrderService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: CCheckOrderDto, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: UCheckOrderDto, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: '点检表统计', description: 'checkItem:点检项 checkList:点检项详情 details.id:对应点检项的id' })
  @Get('check')
  async checkOrderLogs(@Query() dto: CheckOrderListDto) {
    return this.service.checkOrderLogs(dto)
  }
}
