...
import { CRepairOrderDto, FindPaginationDto, RepairActionDto, URepairOrderDto } from './repairOrder.dto'
import { Sequelize } from 'sequelize-typescript'
import { CurrentPage } from '@core/decorator/request'

@ApiTags('维修单')
@ApiBearerAuth()
@AdminAuth('repairOrder')
export class RepairOrderController {
  constructor(private readonly service: RepairOrderService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: CRepairOrderDto, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: URepairOrderDto, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: '维修操作' })
  @Post('action')
  async action(@Body() dto: RepairActionDto, @Req() req) {
    let { factoryCode, loadModel } = req
