...
@ApiTags('排班计划')
@ApiBearerAuth()
@AdminAuth('schedulePlan')
export class SchedulePlanController {
  constructor(private readonly service: SchedulePlanService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: CSchedulePlanDto, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: USchedulePlanDto, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
