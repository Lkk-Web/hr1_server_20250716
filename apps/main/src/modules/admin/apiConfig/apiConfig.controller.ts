...
export class ApiConfigController {
  constructor(private readonly service: ApiConfigService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: CApiConfigDto, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: UApiConfigDto, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
