...
import { CEquipmentDTO, FindPaginationDto, UEquipmentDTO } from './equipment.dto'
...
export class EquipmentController {
  constructor(private readonly service: EquipmentService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: CEquipmentDTO, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: UEquipmentDTO, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
