...
@ApiTags('保养方案')
@ApiBearerAuth()
@AdminAuth('maintenancePlan')
export class MaintenancePlanController {
  constructor(private readonly service: MaintenancePlanService, private readonly sequelize: Sequelize) {}
  @ApiOperation({ summary: '创建' })
  @HttpCode(HttpStatus.OK)
  @Post('/')
  async create(@Body() dto: CMaintenancePlanDto, @Req() req) {
  ...
  @Put(':id')
  async edit(@Body() dto: UMaintenancePlanDto, @Param() params, @Req() req) {
    let { factoryCode, loadModel } = req
    const { id } = params
