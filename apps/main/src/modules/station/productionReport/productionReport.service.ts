...
import { CProductionReportDto, UProductionReportDto, FindPaginationDto, batchDto, auditDto } from './productionReport.dto'
...
export class ProductionReportService {
            ...
            if (pop.planCount == pop.goodCount + dto.reportQuantity) {
              ...
              if (pop1.dataValues.actualStartTime) {
                ...
                let popTemp = await POP.findOne({
                  where: { productionOrderTaskId: dto.productionOrderTaskId, id: pop1.id + 1 },
                  order: [['id', 'ASC']],
  ...
  public async find(id: number, loadModel) {
    ...
    const order = await ProductionOrder.findByPk(result.productionOrderTaskId)
    let count = 0
    ...
    if (dto.ids != undefined) {
      for (const id of dto.ids) {
        ...
        if (report) {
          ...
          if (dto.status === '已审核') {
            const order = await ProductionOrder.findByPk(report.productionOrderTaskId, {
              include: [
                {
                  association: 'bom',
                  attributes: ['id', 'parentMaterialCode', 'remark', 'version', 'quantity', 'formData'],
              ...
              if (report.accountingType === '计件') {
                ...
                const temp = await PerformanceDetailed.findOne({
                  where: {
                    ...
                    productionOrderTaskId: report.productionOrderTaskId,
                    userId: perUser.id,
                  },
                })
                if (!temp) {
