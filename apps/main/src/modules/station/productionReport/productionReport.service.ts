...
import { PerformanceDetailed } from '@model/pp/performanceDetailed.model'
...
export class ProductionReportService {
            ...
            if (pop.planCount == pop.goodCount + dto.reportQuantity) {
              ...
              if (pop1.dataValues.actualStartTime) {
                ...
                let popTemp = await POP.findOne({
                  where: { productionOrderId: dto.productionOrderId, id: pop1.id + 1 },
                  order: [['id', 'ASC']],
  ...
  public async find(id: number, loadModel) {
    ...
    const order = await ProductionOrder.findByPk(result.productionOrderId)
    let count = 0
