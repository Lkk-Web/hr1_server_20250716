...
export class ProductionReportTwoService {
          ...
          include: [
            {
              ...
              include: [
                { association: "parentMaterial", attributes: ['id', 'unit'] }
              ]
            }
          ]
        },
    ...
    for (let i = 0; i < tasks.length; i++)  {
      ...
      if (nextPop) {
        if(nextPop.status=='未开始'){
          ...
          await ProductionOrder.update({ currentProcess: nextPop.dataValues.process.processName }, {
            where: { id: task.productionOrderId },
          });
        }
        //更新接收数
        if(!task.isInspection&&nextPop.processTaskId)await ProcessTask.update({
  ...
  private async createReportUserDuration(configList:PadRegisterUserDto[],tasks:ProcessTask[],productionReports:ProductionReport[]) {
    const userDurations = await ReportUserDuration.bulkCreate(configList.map(v => ({
      userId: v.userId,
      duration: v.duration,
    })));
    //实际总时长
  ...
  public async produceStore(dto: PickingOutboundDto,transaction:Transaction=null) {
    try {
      ...
      if(order.billType==STORAGE_TYPE.report){
        ...
        const dataInst = await KingdeeeService.getList('PRD_INSTOCK', 'FID,FEntity_FEntryID,FMoBillNo,FMoEntrySeq,FMaterialId.FNumber,FInStockType,FUnitID.FNumber,FBaseUnitId.FNumber,FOwnerTypeId,FStockId.FNumber,FStockStatusId.FNumber', `FID=${rkid}`)
